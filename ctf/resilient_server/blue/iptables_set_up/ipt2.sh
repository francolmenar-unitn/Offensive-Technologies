...
sudo iptables -A FORWARD -i $router_interface -o $server_interface -p tcp -m length --length 234 -j DROP
...
#sudo iptables -A FORWARD -i $router_interface -s 10.1.1.2,10.1.2.2,10.1.3.2,10.1.4.2 -d 10.1.5.2 -o $server_interface -p tcp -m string --string "GET /?" --algo bm -j DROP
#sudo iptables -A FORWARD -i $router_interface -s 10.1.1.2,10.1.2.2,10.1.3.2,10.1.4.2 -d 10.1.5.2 -o $server_interface -p tcp --dport 80 --syn -m conntrack --ctstate NEW -j RATE-LIMIT
sudo iptables -A FORWARD -i $router_interface -s 10.1.2.2 -d 10.1.5.2 -o $server_interface -p tcp --sport 1234 -j DROP
sudo iptables -A FORWARD -i $router_interface -s 10.1.2.2 -d 10.1.5.2 -o $server_interface -p tcp -m string --string "GET /?" --algo bm -j DROP
#sudo iptables -A FORWARD -i $router_interface -s 10.1.2.2 -d 10.1.5.2 -o $server_interface -p tcp -m string --string "GET /" --algo bm -j ACCEPT
sudo iptables -A FORWARD -i $router_interface -s 10.1.2.2 -d 10.1.5.2 -o $server_interface -p tcp --dport 80 --syn -m conntrack --ctstate NEW -j RATE-LIMIT
...
sudo iptables -A RATE-LIMIT --match hashlimit --hashlimit-mode srcip --hashlimit-upto 30/sec --hashlimit-burst 40 --hashlimit-name conn_rate_limit -j ACCEPT
